...
<body>
    ...
    <main id="content" class="sub member checkerms">
        <!-- 작업 영역 -->
        ...
        <div class="container">
            ...
            <div class="termbox-area">
                <div class="checkbox-area">
                    <input id="chbox-terms" class="checkbox-d" type="checkbox" />
                    <label for="chbox-terms">이용약관</label>
                    <a href="#" class="btn btn-bo pull-right">내용보기</a>
                </div>
                <div class="txt-area">
                    <textarea name="" id="" class="wful" readonly>
...
제2조(정의)
① “몰”이란 엠코르셋 주식회사 회사가 재화 또는 용역(이하 “재화 등”이라 함)을 이용자에게 제공하기 위하여 컴퓨터 등 정보통신설비를 이용하여 재화 등을 거래할 수 있도록 설정한 가상의 영업장을 말하며, 아울러 사이버몰을 운영하는 사업자의 의미로도 사용합니다.
...
① “몰”은 이 약관의 내용과 상호 및 대표자 성명, 영업소 소재지 주소(소비자의 불만을 처리할 수 있는 곳의 주소를 포함), 전화번호·모사전송번호·전자우편주소, 사업자등록번호, 통신판매업 신고번호, 개인정보보호책임자 등을 이용자가 쉽게 알 수 있도록 엠코르셋 주식회사 사이버몰의 초기 서비스화면(전면)에 게시합니다. 다만, 약관의 내용은 이용자가 연결화면을 통하여 볼 수 있도록 할 수 있습니다.
...
본 약관은 2014년12월29일부터 적용됩니다.
                    </textarea>
                </div>
                <div class="checkbox-area">
                    <input id="chbox-privacy" class="checkbox-d" type="checkbox" />
                    <label for="chbox-privacy">개인정보 수집 및 이용안내</label>
                </div>
                <div class="txt-area">
                    <table class="wful">
                        <colgroup>
                ...
                <div class="checkbox-area">
                    <input id="chbox-receive" class="checkbox-d" type="checkbox" />
                    <label for="chbox-receive">마케팅 수신 동의</label>
                    <span>
                        (
                        <input id="chbox-email" class="checkbox-d" type="checkbox" />
                        <label for="chbox-email">이메일</label>
                        <input id="chbox-sms" class="checkbox-d" type="checkbox" />
                        <label for="chbox-sms">SMS</label>
                        )
                    </span>
                </div>
                <div class="txt-area">
                    쇼핑몰에서 제공하는 신상품 소식/ 할인쿠폰을 무상으로 보내드립니다! 단, 상품 구매 정보는 수신동의 여부 관계없이 발송됩니다.<br>
                    <span class="fpoint">제공 동의를 하지 않으셔도 서비스 이용에는 문제가 없습니다.</span>
                </div>
            </div>

            <p class="text-center mt30">
                <input id="chbox-all" class="checkbox-d" type="checkbox" />
                <label for="chbox-all">전체동의 합니다</label>
            </p>

            <div class="btn-area text-center mt50 mb80">
                <a href="#" class="btn btn-bg">취소</a>
                <a href="/member/register.php" class="btn btn-p">다음</a>
            </div>
...
$("input").click(function () {
    var ch_id = $(this).attr('id'),
        ch_val = ($(this).prop('checked')) ? true : false,
        str = '';

    if(ch_id == 'chbox-all' || ch_id == "chbox-receive") {
        str = (ch_id == "chbox-all") ? '.checkbox-d' : '#chbox-email, #chbox-sms';
        
        if(ch_val) {
            $(str).not('.checked').prop('checked',true);
        } else {
            $(str).prop('checked',false);
        }
    } else {
        if($(this).parent().find('input:checked').length == 2) {
            $(this).parent().find('input.checked').prop('checked',true);
            $('#chbox-receive').prop('checked',true);
        } else {
            $('#chbox-receive').prop('checked',false);
        }
    }
});
</script>
